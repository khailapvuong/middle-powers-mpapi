...
Literature-weighted composite ranking (full table and per-scheme variants in §11 of the notebook). **The UK and South Korea are the two co-leaders: the UK ranks 1st under all three cardinal schemes (equal, PCA, literature), with South Korea a close 2nd. The weighting-free Borda/Kemeny consensus instead places Korea 1st**, and both carry a Monte Carlo median rank of 2 — best read as a close 1–2 pair, the leader depending on aggregation method. Under the headline literature scheme the UK leads Korea 2.214 to 2.162.

| Rank | Country     | Composite | Rank | Country      | Composite |
| ---- | ----------- | --------- | ---- | ------------ | --------- |
|    1 | UK          |     2.214 |    8 | Canada       |     1.350 |
|    2 | South Korea |     2.162 |    9 | Sweden       |     1.037 |
|    3 | France      |     1.958 |   10 | UAE          |     0.883 |
|    4 | Japan       |     1.769 |   11 | Saudi Arabia |     0.813 |
|    5 | EU          |     1.768 |   12 | Israel       |     0.135 |
|    6 | Singapore   |     1.767 |   13 | India        |     0.104 |
|    7 | Germany     |     1.398 |   14 | Taiwan       |     0.060 |
...
- The **top-5 set** {UK, South Korea, France, EU, Japan} is identical across all three weighting schemes (equal, PCA-derived, literature-informed). Within it, the UK ranks 1st under all three cardinal schemes and South Korea a close 2nd; the two co-lead (Korea is 1st under the weighting-free Borda/Kemeny consensus, §12.11).
...
- **Robustness** (`outputs/robustness_summary_with_ci.csv`): every perturbation reports Spearman ρ ≥ 0.96. Under the Bonett–Wright Spearman SE, the equal-vs-PCA, z-vs-min-max, and equal-vs-literature perturbations all clear the stricter 0.85 bar (95% CI lower bounds 0.877, 0.861, and 0.877); only the classifier-sensitivity check (ρ = 0.960, CI lower bound 0.845) sits below 0.85 — it clears the 0.70 robustness floor but not the stricter classifier bar, so the ranking is robust to weighting and normalisation and borderline to the C3 concept choice (this is why H4 is *partially* supported).

### Tier stability is partial — single-slot cycling

Per `outputs/h6_set_membership.json` (computed in §16.11):

- **All five baseline top-5 countries** appear in the top-5 in ≥ 70% of the 10,000 Monte Carlo draws (GBR 98.6%, KOR 97.9%, FRA 86.8%, JPN 83.3%, EU 76.8%), so the top-5 tier passes its per-member criterion.
- **The bottom-3 tier is weaker:** no bottom-3 member clears 80% (India 75.0%, Taiwan 60.9%, Israel 49.4%), so the bot-3 per-member criterion fails — Israel in particular swaps in and out of the bottom-3 with Saudi Arabia.
- **Within-1-swap tolerance** (at most one country differs from the baseline tier): 93% of top-5 draws, 66% of bot-3 draws.
- **Exact set-match**: only ~51% of top-5 draws and ~23% of bot-3 draws because the 5th and 12th slots cycle.
...
- **The UK and Korea both have Monte Carlo median rank 2** (UK p10–p90 1–4, IQR 2; Korea 1–4, IQR 1 — the tightest top distribution). Under point scores the UK is 1st under all three cardinal schemes, South Korea a close 2nd (Korea leads the weighting-free Borda/Kemeny consensus).
- The **{France, Japan, EU} block at positions 3–5** is weight-sensitive: France 1.958, Japan 1.769, EU 1.768 — Monte Carlo gives France and the EU a median rank of 4 and Japan a median of 5 (the EU's wide spread, p10–p90 = 1–7, reflects its partly-synthetic row).
...
flowchart LR
  ...
  Sens["§12 sensitivity<br/>Monte Carlo · LOO · alt-norm · classifier<br/>+ ε-grid · collinearity · coding-collapse<br/>+ country-jackknife · weighting-free consensus<br/>+ compute robustness · aggregation sensitivity"]
...
| **Infrastructure Posture** | ICT infrastructure, connectivity, platform/cloud presence (paper names it *Compute and Infrastructure Posture*; a dedicated TOP500 **compute** indicator is sourced and tested as a robustness check in §12.12 — it leaves the ranking ~unchanged and is reported there rather than folded into the headline) | I1 ITU IDI · I2 Broadband · I3 Secure servers · I5 ND-GAIN Readiness (plus I4 = TOP500 compute, §12.12) |
...
- **Three weighting schemes reported side-by-side** — equal · PCA-derived · literature-informed (author-assigned weights reflecting the relative emphasis of GMF Pivotal Powers + Chatham House Sovereign AI + Tortoise Global AI Index; not a formal expert elicitation — see §17.6).
...
- **C5×G1 axis-reassignment (§12.7)** — recompute the ranking with C5 moved to governance, under both equal-weight (axis-assignment isolation) and headline-literature weights, to bound the ordering impact of the C5/G1 cross-axis double-count.
...
- **Weighting-free consensus (§12.11)** — Borda and exact Kemeny–Young aggregation of the 14 indicator ballots with no cardinal weights (Borda ρ = 0.977, Kemeny ρ = 0.965 vs the literature ranking) — a weight-free corroboration of the headline.
- **Compute-indicator robustness (§12.12)** — sources a dedicated, hosting-attributed compute measure (TOP500 supercomputer-system count, Nov 2025) and tests adding it to the infrastructure axis: ρ = 0.996 vs the headline (only an EU↔Japan 4↔5 swap). Reported as a robustness check, not folded into the headline, because it tips the silhouette-selected k to 2 where average-linkage clustering degenerates (an artefact that would fail the H3 criterion). Closes the "Compute and Infrastructure Posture" construct gap on the evidence.
- **Aggregation-function sensitivity (§12.13)** — re-rank under an arithmetic across-axis mean instead of the geometric mean to bound how much the no-axis-compensates aggregation choice (the single most consequential methodological choice for the lower tier) drives the table: Spearman ρ = 0.978 vs the geometric headline (Bonett–Wright 95% CI [0.911, 0.995]). The top-3 are invariant; the rest reorders modestly (max single-country shift = 2 — Japan and the near-tied Singapore swap across the 4–6 boundary, India↔Taiwan at the bottom). `outputs/aggregation_sensitivity.csv`.
...
| **H3** | The 14 middle powers do not cluster homogeneously. | **SUPPORTED** | k-means silhouette = 0.42 at k = 3; ARI = 1.0 against all three hierarchical linkages (Ward, complete, average); MDS embedding preserves distance rank at ρ = 0.91. |
| **H4** | The headline ranking is robust to methodological perturbation. | **PARTIALLY SUPPORTED** | Under the Bonett–Wright Spearman SE the equal-vs-PCA, z-vs-min-max, and equal-vs-literature perturbations all clear the stricter 0.85 bar (CI lower bounds 0.877 / 0.861 / 0.877); only the classifier-sensitivity check (ρ = 0.960, CI lower bound 0.845) clears 0.70 but sits just below the stricter 0.85 bar applied to the classifier — robust on weighting/normalisation, borderline on the C3 concept choice. |
| **H5** | Top-5 not driven by any single analyst-coded indicator. | **SUPPORTED** | Top-5 set changes by at most one country — the near-tied EU↔Singapore 5th slot swaps under the C5→binary collapse, within the §1.4 ≤1-country tolerance; the §12.8 judgment-collapse leaves the headline essentially unchanged (min ρ = 0.9956, max single-country shift = 1). |
| **H6** | Top-5 and bot-3 tier membership stable under weight perturbation. | **PARTIALLY SUPPORTED** | All five baseline top-5 members appear in the top-5 in ≥ 70% of 10,000 Monte Carlo draws (PASS); no bottom-3 member clears 80% (India 75.0%, Taiwan 60.9%, Israel 49.4%), so the bot-3 per-member criterion fails — Israel is the least stable, cycling with Saudi Arabia. |
...
Several additional sensitivity tests were added in response to anticipated reviewer concerns. None reorders the top-5 set; each surfaces a quantitative answer to a specific framework concern:

| Check | Concern addressed | Result | Persisted to |
|---|---|---|---|
| **§12.2.1 C4-drop sensitivity** | Does the headline top tier depend on a single Stanford-Top-15 patent figure? | Spearman ρ(baseline_rank, C4-drop_rank) = +0.9956 (Fisher-z 95% CI [+0.981, +0.999]). The top-5 set is preserved; Korea holds rank 2 (Δ = 0). | Printed in §12.2.1 (not persisted to CSV). |
| **§13.5 V-Dem polarity sensitivity** | Paper §2.4 argues democracies are *more* exposed to influence operations — does flipping V-Dem polarity within the influence-ops vector reshape the per-vector ranking? | Spearman ρ(baseline_infl_rank, V-Dem-flipped_infl_rank) = +0.9341 (Bonett–Wright Fisher-z 95% CI [+0.754, +0.984]). Flipping V-Dem polarity (democracy read as vulnerability rather than resilience) moves lower-democracy states up and higher-democracy states down, as paper §2.4 predicts: UAE rises 10 → 7, Saudi Arabia 11 → 9 and Singapore 3 → 1, while the UK falls 1 → 2, Korea 2 → 3, Germany 8 → 10, Canada 9 → 11 and Sweden 7 → 8. France, Japan, EU, India, Israel and Taiwan are unchanged. | `outputs/vdem_polarity_sensitivity.csv`. |
| **§16.11(d) H6 mechanical verdict** | The §1.4 pre-registered criterion for H6 was prose-only; encode the asymmetric thresholds (≥ 4/5 top-5 members in ≥ 70% of draws AND ≥ 2/3 bot-3 members in ≥ 80% of draws) as a mechanical PASS/FAIL. | **Combined FAIL** (top-5 PASS, bot-3 FAIL). Top-5: 5/5 members in tier in ≥ 70% of draws (UK 98.6%, Korea 97.9%, France 86.8%, Japan 83.3%, EU 76.8%) → PASS at 4/5; bot-3: 0/3 members in tier in ≥ 80% (India 75.0%, Taiwan 60.9%, Israel 49.4%) → FAIL at 2/3. The overall H6 verdict is PARTIALLY SUPPORTED: the top tier is stable, but the bottom tier is weight-sensitive (no member clears 80%). | `outputs/h6_set_membership.json`, key `per_country_in_tier_mechanical_verdict`. |
| **§12.6 geometric-shift ε sensitivity** | The composite shifts z-scored axes by `+ ε` (ε = 1e-3) before the geometric mean; ε is the one aggregation constant §12.1–§12.5 never perturb, and the country at an axis minimum contributes `log(ε)`. Does the bottom tier depend on it? | Across ε ∈ {1e-4, 1e-3, 1e-2, 1e-1, 1.0} the **ranking is near-invariant** (Spearman ρ = 1.000 for ε ≤ 0.01; ρ = 0.987 at ε = 0.1; ρ = 0.982 at ε = 1.0). But the **bottom-tier composite values are an ε-artifact**: Taiwan's composite spans 0.024 → 1.56 and Israel's 0.067 → 1.70 across the grid (~65× / ~25×). **Cite the bottom tier as ranks, not magnitudes.** | `outputs/eps_sensitivity.csv`; printed in §12.6. |
| **§12.7 C5×G1 axis-reassignment** | C5 (AISI presence, capacity) and G1 (AI strategy, governance) are near-collinear (r = 0.965), so the shared AI-safety-institution signal loads into two axes. How much of the ranking is the double-count? | Reassigning C5 to governance (the §17.13 alternative #1) bounds the double-count under both conventions. **Equal within-axis weights** (isolating the axis-assignment effect): Spearman ρ = +0.9824 (Bonett–Wright Fisher-z 95% CI [+0.928, +0.996]), **top-5 set unchanged**, max single-country shift 1. **Headline literature weights**: ρ = +0.9824, with the **top three (UK, Korea, France) and the entire bottom tier unchanged** and only a Japan↔Singapore swap inside the already weight-sensitive 4–6 block (max shift 2). The double-count inflates the cardinal Tier-1↔Tier-3 *gap* but does not drive the *ordering*. | `outputs/collinearity_recode.csv` (equal) + `outputs/collinearity_recode_literature.csv` (literature); printed in §12.7. |
...
| **§12.13 geometric-vs-arithmetic aggregation** | The across-axis geometric mean enforces the paper's no-axis-compensates logic; §12.1–§12.4 perturb weights/normalisation/classifier but never the aggregation *function* itself. How much does the headline depend on it vs an arithmetic mean? | Spearman ρ = 0.978 vs the geometric headline (Bonett–Wright 95% CI [0.911, 0.995]); the **top-3 are invariant** and the rest reorders modestly (max single-country shift = 2 — Japan and the near-tied Singapore swap across the 4–6 boundary, India↔Taiwan at the bottom). The aggregation function is the single most consequential methodological choice for the lower tier. | `outputs/aggregation_sensitivity.csv`; printed in §12.13. |
...
### External concurrent validity (§16.12)

The checks above are *internal* (re-perturbations of M-PAPI's own pipeline). To guard against circularity, none of M-PAPI's 14 indicators is itself a published readiness score — so the composite can be benchmarked against an independent index of the same latent construct. §16.12 correlates the literature-weighted composite against the **Oxford Insights Government AI Readiness Index 2024** (GAIRI) — an independently constructed index (188 jurisdictions; 40 indicators across Government, Technology-Sector, and Data-&-Infrastructure pillars), sharing no inputs with M-PAPI. The two rankings agree strongly: **Spearman ρ = 0.852 (95% CI [0.492, 0.963], n = 13** — the EU is excluded, as GAIRI publishes no EU-level row). Strong agreement despite disjoint inputs supports M-PAPI's concurrent validity (`outputs/external_validity.csv`; rendered as `figures/fig16_12_external_validity.png`).

## Notebook structure

The 190-cell notebook follows the OECD/JRC 10-step composite-indicator process and emits a Power BI consumption layer:
...
| **§7.1** | Per-cell provenance ledger — raw (as-sourced) value, applied transform, value used, observed / inlined / analyst-coded / imputed status, and source + URL per cell (`outputs/indicator_provenance.csv`); `raw_value` reconciles directly against the cited source, `value_used` is the post-transform/post-imputation value the index consumes |
...
- **Random seed** — `SEED = 20260506` (defined in §3.1), passed explicitly to all eight stochastic entry points:
  ...
  - `np.random.default_rng(SEED + 202)` for the §13.6 per-vector vulnerability-overlay-weight Monte Carlo
