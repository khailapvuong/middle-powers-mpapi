...
- The **{France, Japan, EU} block at positions 3–5** is weight-sensitive: France 1.958, Japan 1.769, EU 1.768 — Monte Carlo gives all three a median rank of 4 (the EU's wide spread, p10–p90 = 1–7, reflects its partly-synthetic row).
...
|---|---|---|---|
| **§12.2.1 C4-drop sensitivity** | Does the headline top tier depend on a single Stanford-Top-15 patent figure? | Spearman ρ(baseline_rank, C4-drop_rank) = +0.9956 (Fisher-z 95% CI [+0.981, +0.999]). The top-5 set is preserved; Korea (rank 2) slips at most one place but its Tier-1 placement survives. | LOO row in `outputs/sensitivity_ranks.csv`; printed in §12.2.1. |
...
outputs/                      — 37 outputs (34 CSV + 3 JSON) of index, sensitivity, and verdict tables
