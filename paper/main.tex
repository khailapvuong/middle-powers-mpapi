...
\textbf{Cross-axis collinearity.} C5 (AISI presence) and G1 (AI-strategy comprehensiveness) are near-collinear ($r=0.973$) yet sit in different axes, so a shared institutional signal enters the composite twice. Reassigning C5 to governance leaves the top-five set unchanged (Spearman $\rho=0.996$, maximum single-country shift~1), so the double-count inflates the Tier-1$\leftrightarrow$Tier-3 gap without re-ordering the table; it is nonetheless an acknowledged construct flaw.
