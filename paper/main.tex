...
The raw $14\times14$ matrix is $91.3\%$ observed ($179$ of $196$ cells). Figure~\ref{fig:missing} maps every cell to one of three states: observed, stage-1 axis-mean imputed, or stage-2 column-mean fallback. The largest gap is C4 (AI patents), missing for seven of the fourteen jurisdictions because they fall below the Stanford AI Index's published top-15 cut-off; C4 therefore exceeds the imputation threshold and its missing cells are column-mean filled. Taiwan's four infrastructure indicators are unobserved (a missing-not-at-random exclusion from ITU and World Bank ICT collection) and are likewise column-mean filled---a caveat carried through every downstream figure.
...
\midrule
1  & South Korea           & 2.096 & 2.367 & 2.256 \\
2  & United Kingdom        & 2.147 & 2.490 & 2.256 \\
3  & France                & 1.823 & 2.283 & 1.993 \\
4  & Japan                 & 1.745 & 2.108 & 1.827 \\
5  & European Union        & 1.781 & 2.343 & 1.798 \\
6  & Singapore             & 1.372 & 1.668 & 1.797 \\
7  & Germany               & 1.393 & 1.875 & 1.481 \\
8  & Canada                & 1.420 & 2.084 & 1.431 \\
9  & Sweden                & 1.034 & 1.372 & 1.195 \\
10 & United Arab Emirates  & 0.337 & 0.395 & 0.954 \\
11 & Saudi Arabia          & 0.113 & 0.116 & 0.856 \\
12 & Israel                & 0.360 & 0.936 & 0.144 \\
13 & India                 & 0.081 & 0.101 & 0.109 \\
...
Under the Bonett--Wright standard error, the equal-vs-PCA and normalisation perturbations clear the $0.70$ robustness floor with wide margins (Table~\ref{tab:robust}, Figure~\ref{fig:forest}). The equal-vs-literature perturbation ($\rho=0.960$, CI lower bound $0.845$) and the bibliographic-classifier check ($\rho=0.960$, CI lower bound $0.845$) both clear the $0.70$ floor yet sit below the stricter $0.85$ bar imposed on the classifier (a core measurement choice rather than a downstream weight perturbation). The Monte Carlo rank distributions (Appendix Figure~\ref{fig:rankdist}) show the United Kingdom and South Korea co-leading at a median rank of~2 and the $\{$France, Japan, EU$\}$ block sharing a median rank of~4.
...
\midrule
Weighting: equal vs PCA            & 0.982 & 0.70 & $[0.928,\,0.996]$ & clears \\
Weighting: equal vs literature     & 0.960 & 0.70 & $[0.845,\,0.990]$ & clears \\
Normalisation: $z$ vs min--max     & 0.978 & 0.70 & $[0.911,\,0.995]$ & clears \\
...
Table~\ref{tab:hyp} records the six hypotheses, their numeric pass criteria, and their mechanically resolved verdicts. Two are fully supported (H3, H5) and four are partially supported with named caveats (H1, H2, H4, H6). The partial verdicts surface specific methodological caveats---a near-collinear pair across two axes (C5$\times$G1, Pearson $r=0.973$), governance multidimensionality, two borderline robustness checks (equal-vs-literature and the classifier), and a weight-sensitive bottom tier---rather than a wholesale framework failure. The C5$\times$G1 collinearity is the largest of only five cross-axis indicator pairs at $|r|\geq0.7$ (of 65 pairs).
...
\midrule
H1 & The three axes are empirically separable. & Partial & $5$ of $65$ cross-axis pairs at $|r|\geq0.7$; C5$\times$G1 ($r=0.973$) is the largest contributor. \\
H2 & Each axis carries a one-dimensional signal. & Partial & Infrastructure $p=0.014$, governance $p=0.040$ (both significant); capacity untestable at $n=7$; governance PC1 explains $45.6\%$. \\
H3 & The 14 powers do not cluster homogeneously. & Supported & Silhouette $0.427$ at $k=3$; ARI $=1.0$ vs Ward, complete, and average linkage; MDS distance-rank $\rho=0.919$. \\
H4 & The ranking is robust to perturbation. & Partial & Equal-vs-PCA and normalisation clear with wide margins; equal-vs-literature and classifier both have CI lower bound $0.845$ (below the $0.85$ classifier bar). \\
...
H6 & Top-5 and bottom-3 tier membership are stable. & Partial & $5/5$ top-5 members in tier in $\geq70\%$ of draws; no bottom-3 member reaches $80\%$, Israel only $48.7\%$, cycling with Saudi Arabia. \\
\bottomrule
\end{tabularx}
\end{table}
...
\subsection{Typology}
The silhouette-selected $k=3$ $k$-means partition separates a Tier-1/asymmetric-developed group, a mid group, and India as a singleton; the partition is identical to Ward, complete, and average-linkage hierarchical clustering (ARI $=1.0$ for all three). The MDS embedding (Figure~\ref{fig:mds}) preserves the pairwise-distance rank at $\rho=0.919$ and visually separates the archetypes.

\begin{figure}[t]
\centering
\includegraphics[width=0.86\textwidth]{figures/fig14_3_mds_embedding.png}
\caption{Multidimensional-scaling embedding of the 14 jurisdictions in the full fourteen-indicator space (distance-rank $\rho=0.919$). Colour: archetype; marker shape: AISI Network membership. Source: \texttt{outputs/mds\_coordinates.csv}.}
\label{fig:mds}
\end{figure}
...
The $\varepsilon$-sensitivity, C5-reassignment, and analyst-coded judgment-collapse checks all leave the top-five set intact: the analyst-collapse variants (G1 and C5 individually and both at once) return min Spearman $\rho=0.996$ with at most a single mid-table shift, and the C5-reassignment returns $\rho=0.987$. Cross-axis correlations at $|r|\geq0.7$ number five of sixty-five pairs (C5$\times$G1 $0.973$; C1$\times$G1 $0.779$; C4$\times$I3 $-0.771$; G3$\times$I3 $0.722$; C3$\times$G6 $0.702$). Tier-membership stability over $10{,}000$ Monte Carlo draws: all five top-5 members in tier $\geq70\%$ of draws (UK $98.6\%$, Korea $98.0\%$, France $85.0\%$, Japan $81.0\%$, EU $75.8\%$); no bottom-3 member reaches $80\%$ (India $75.8\%$, Taiwan $74.2\%$, Israel $48.7\%$); exact-set-match $47\%/28\%$ and within-one-swap $92\%/71\%$ for the top-5/bottom-3.
...
\midrule
South Korea           &  0.74 & 1  &  0.83 & 1  &  0.68 & 2 \\
United Kingdom        &  0.69 & 2  &  0.82 & 2  &  0.70 & 1 \\
Singapore             &  0.47 & 3  &  0.26 & 6  &  0.54 & 3 \\
European Union        &  0.47 & 4  &  0.57 & 4  &  0.23 & 6 \\
France                &  0.44 & 5  &  0.60 & 3  &  0.37 & 4 \\
Japan                 &  0.32 & 6  &  0.36 & 5  &  0.32 & 5 \\
Canada                &  0.07 & 7  &  0.15 & 7  & -0.10 & 9 \\
Germany               &  0.02 & 8  &  0.05 & 8  & -0.02 & 8 \\
Sweden                & -0.09 & 9  & -0.30 & 9  &  0.18 & 7 \\
United Arab Emirates  & -0.26 & 10 & -0.48 & 10 & -0.17 & 10 \\
Saudi Arabia          & -0.44 & 11 & -0.57 & 11 & -0.38 & 11 \\
Israel                & -0.64 & 12 & -0.72 & 13 & -0.64 & 13 \\
Taiwan                & -0.75 & 13 & -0.99 & 14 & -0.49 & 12 \\
...
\caption{Hierarchical clustering at $k=3$ under Ward, complete, and average linkage. Ward, complete, and average partitions are identical to the $k$-means partition (ARI $=1.0$). Source: \texttt{outputs/hierarchical\_ari.csv}.}
\label{fig:dendro}
\end{figure}
