...
\usepackage{xurl} % allow long URLs (bibliography) to break across lines
...
\begin{abstract}
\noindent The working paper \emph{AI-Proliferation and Middle Powers} argues that fourteen named middle powers differ along three axes---\emph{capacity depth}, \emph{governance orientation}, and \emph{infrastructure posture}---and that the tractability of preparedness actions depends on a country's position along them, yet it never empirically positions any country. This companion study closes that gap with the Middle-Power AI-Proliferation Preparedness Index (M-PAPI): a reproducible composite indicator built from fourteen publicly sourced indicators following the OECD/JRC \emph{Handbook on Constructing Composite Indicators}. We report a literature-weighted ranking in which the United Kingdom ($2.214$) and South Korea ($2.162$) co-lead---the UK first under all three cardinal schemes (equal, PCA, and literature), with South Korea a close second, while the weighting-free Borda/Kemeny consensus instead places Korea first---and India ($0.104$) and Taiwan ($0.060$) trail; the top-five set $\{$UK, Korea, France, EU, Japan$\}$ is identical across equal-, PCA-, and literature-weighting schemes. Six hypotheses are pre-registered with numeric pass criteria and resolved mechanically: two are fully supported (cluster heterogeneity; insensitivity of the top tier to any single analyst-coded indicator) and four are partially supported with specific, named caveats (cross-axis separability, axis dimensionality, ranking robustness, and tier-membership stability). A vulnerability overlay maps axis scores onto the paper's three attack vectors, counterfactual scenarios quantify per-country policy leverage, and an enabler-readiness matrix extends the paper's $\S$4 preparedness checklist into a structural readiness score for every country. The ranking is strongly rank-concordant with the independent Oxford Insights Government AI Readiness Index 2024 (Spearman $\rho=0.852$ over the thirteen jurisdictions both cover), evidence of external concurrent validity. Within a pinned software environment, all outputs regenerate byte-for-byte under a fixed seed and an offline data snapshot. The index empirically grounds the paper's $\S$1--$\S$2 premise---the middle powers are differentiable, and the most-exposed tier is identifiable---and structurally operationalises the paper's $\S$4 tractability claim, while explicitly declining the $\S$3 incident-level claims it cannot yet test.
\end{abstract}
...
\paragraph{Scope.} M-PAPI operationalises the paper's $\S$1 (axes and country set) and $\S$2 (attack vectors) directly, and the $\S$4 \emph{tractability} claim structurally. The paper's $\S$3 (trigger-event ladder) is \emph{not} operationalised: a country-year, AI-attributed incident dataset would be required, and none exists for these fourteen jurisdictions across the three attack vectors without heuristic estimation (Section~\ref{sec:limits}). The $\S$4 detection/escalation/mitigation checklist is operationalised \emph{partially}: its actions appear both as the counterfactual policy scenarios of Section~\ref{sec:results} and, more generally, as the enabler-readiness matrix of Section~\ref{subsec:readiness}, which scores each country's structural readiness for the checklist actions against the indicators proxying each action's required enablers---not as country-year preparedness outcomes. The index measures \emph{relative} position within the fourteen-country reference frame; it does not claim that a high score means a country is ``safe.''
...
Table~\ref{tab:axes} reproduces the paper's axis definitions and lists the indicators assigned to each. The fourteen indicators comprise five \emph{capacity-depth} measures, five \emph{governance-orientation} measures, and four \emph{infrastructure-posture} measures. The working paper labels the third axis \emph{Compute and Infrastructure Posture}; the headline operationalises it as \emph{Infrastructure Posture} from ICT measures, while a dedicated, hosting-attributed compute indicator (the TOP500 supercomputer-system count) is sourced and tested in Appendix~\ref{app:robust}---it leaves the ranking essentially unchanged (Spearman $\rho=0.996$) and is reported as a robustness check rather than folded into the pre-registered headline (Section~\ref{sec:limits}).

\begin{table}[t]
\centering
\caption{The three axes of the working paper's framework and the M-PAPI indicators assigned to each. The working paper names the third axis \emph{Compute and Infrastructure Posture}; the headline uses the ICT measures listed, and a dedicated TOP500 compute indicator is sourced and tested as a robustness check (Appendix~\ref{app:robust}). Indicator construction and sources are detailed in Section~\ref{sec:data}.}
\label{tab:axes}
\footnotesize
...
The raw $14\times14$ matrix is $93.9\%$ observed ($184$ of $196$ cells). Figure~\ref{fig:missing} maps every cell to one of three states: observed, stage-1 axis-mean imputed, or stage-2 column-mean fallback. The largest gap is C4 (AI patents), missing for six of the fourteen jurisdictions because they fall below the Stanford AI Index's published top-15 cut-off; C4 therefore exceeds the imputation threshold and its missing cells are column-mean filled. Taiwan's four infrastructure indicators are unobserved (a missing-not-at-random exclusion from ITU and World Bank ICT collection) and are likewise column-mean filled---a caveat carried through every downstream figure.

\begin{figure}[t]
\centering
\includegraphics[width=0.96\textwidth]{figures/fig5_4_missingness_heatmap.png}
\caption{Per-cell observability of the $14\times14$ raw indicator matrix. Blue: observed; amber: stage-1 axis-mean imputed; red: stage-2 column-mean fallback (missing-not-at-random). C4 patents (six jurisdictions) and Taiwan's four infrastructure cells dominate the red region. Source: \texttt{outputs/missingness\_audit.csv}.}
\label{fig:missing}
\end{figure}
...
Three schemes are reported side by side: \textbf{equal} (uniform within and across axes); \textbf{PCA-derived} (within-axis weights proportional to squared first-principal-component loadings, computed at run time); and \textbf{literature-informed} (Table~\ref{tab:indicators}; across-axis weights $0.40/0.30/0.30$), round-number weights assigned by the authors to reflect the relative emphasis the GMF \emph{Pivotal Powers} \citep{dehoopscheffer2024pivotal}, Chatham House sovereign-AI \citep{sandoval2026sovereign}, and Tortoise Global AI Index \citep{tortoise2023globalai} frameworks place on each indicator family --- not a formal expert elicitation; their influence is bounded by the sensitivity battery (Section~\ref{sec:methods}) and the weighting-free Borda/Kemeny consensus. The PCA scheme is presented as a sensitivity benchmark only: at $n=14$ its loadings are unstable, and on the governance axis it effectively zeroes the cybersecurity indicator (G2) and reallocates its mass chiefly to G3 and G6.

\subsection{Multivariate diagnostics}
We assess whether each axis coheres as a one-dimensional construct using within-axis correlations, Cronbach's $\alpha$ \citep{cronbach1951alpha}, and Horn's parallel analysis \citep{horn1965parallel} (Figure~\ref{fig:pa}). Parallel analysis compares each axis's observed first-component explained-variance ratio against a $95$th-percentile permutation null over $10{,}000$ shuffles. The infrastructure and governance axes clear the null ($p=0.014$ and $p=0.040$); the capacity axis is fit on only eight complete cases---below the ten-case testability floor---so its parallel-analysis verdict is suppressed as unstable (its $p$ swings from $\approx0.08$ at $n=7$ to $\approx0.01$ at $n=8$). Cronbach's $\alpha$ is $0.760$ (capacity), $0.579$ (governance, below the conventional $0.7$ threshold), and $0.722$ (infrastructure).
...
Table~\ref{tab:headline} reports the composite under all three schemes. Under the literature scheme the United Kingdom ($2.214$) and South Korea ($2.162$) co-lead---the UK ranks first under all three cardinal schemes (equal $2.108$, PCA $2.487$, literature $2.214$), with South Korea a close second; the weighting-free Borda and Kemeny consensus instead place Korea first, and both carry a Monte Carlo median rank of $2$, so the pair is best read as a close 1--2 pair with the leader depending on aggregation method---and India ($0.104$) and Taiwan ($0.060$) trail. The top-five set $\{$UK, Korea, France, EU, Japan$\}$ is identical across all three schemes; the bottom-three set contains $\{$India, Taiwan$\}$ under every scheme, with the third slot Israel under the literature scheme and Saudi Arabia under the equal and PCA schemes. Figure~\ref{fig:typology} positions the countries on the capacity $\times$ infrastructure plane, colour-coded by archetype and shape-coded by AISI Network membership.
...
\midrule
1  & United Kingdom        & 2.108 & 2.487 & 2.214 \\
2  & South Korea           & 2.039 & 2.287 & 2.162 \\
3  & France                & 1.790 & 2.287 & 1.958 \\
4  & Japan                 & 1.705 & 2.068 & 1.769 \\
5  & European Union        & 1.761 & 2.339 & 1.768 \\
6  & Singapore             & 1.346 & 1.668 & 1.767 \\
7  & Germany               & 1.329 & 1.826 & 1.398 \\
8  & Canada                & 1.357 & 2.008 & 1.350 \\
9  & Sweden                & 0.875 & 1.278 & 1.037 \\
10 & United Arab Emirates  & 0.314 & 0.380 & 0.883 \\
11 & Saudi Arabia          & 0.108 & 0.115 & 0.813 \\
12 & Israel                & 0.335 & 0.899 & 0.135 \\
13 & India                 & 0.078 & 0.100 & 0.104 \\
...
Under the Bonett--Wright standard error, the equal-vs-PCA, equal-vs-literature, and normalisation perturbations all clear the $0.70$ robustness floor with comfortable margins, and their CI lower bounds ($0.877$, $0.877$, and $0.861$) also clear the stricter $0.85$ bar (Table~\ref{tab:robust}, Figure~\ref{fig:forest}). Only the bibliographic-classifier check ($\rho=0.960$, CI lower bound $0.845$) sits just below the $0.85$ bar imposed on the classifier (a core measurement choice rather than a downstream weight perturbation), while still clearing the $0.70$ floor. The Monte Carlo rank distributions (Appendix Figure~\ref{fig:rankdist}) show the United Kingdom and South Korea co-leading at a median rank of~2, France and the EU sharing a median rank of~4, and Japan at median~5.
...
\midrule
Weighting: equal vs PCA            & 0.969 & 0.70 & $[0.877,\,0.993]$ & clears \\
Weighting: equal vs literature     & 0.969 & 0.70 & $[0.877,\,0.993]$ & clears \\
...
\paragraph{Country influence and weighting-free consensus.} Two further checks (Section~\ref{sec:methods}) confirm the ordering is neither a single-country artefact nor a weighting artefact. The leave-one-\emph{country}-out jackknife shows that dropping any one jurisdiction shifts the retained thirteen by at most three rank positions (worst case: dropping India or Taiwan), the retained-order Spearman $\rho$ never falls below $0.945$, and the $k$-means typology is identical (ARI $=1.0$) for twelve of the fourteen drops---dipping only when India (ARI $=0.61$) or Taiwan (ARI $=0.47$) is removed. The weighting-free consensus is equally reassuring: aggregating the fourteen indicator ballots with \emph{no} cardinal weights reproduces the literature ordering under both the Borda count (Spearman $\rho=0.977$, $95\%$ CI $[0.906,\,0.994]$) and the exact Kemeny--Young median ranking ($\rho=0.965$, $[0.861,\,0.991]$). South Korea is first under both and the top-five and bottom-three sets are preserved under Borda; the only consequential disagreement is that the Kemeny median elevates the EU into the second slot (literature fifth)---a weight-independent reminder that the 2--5 block is the table's soft region (Appendix Figures~\ref{fig:loocountry} and~\ref{fig:rankagg}).
...
H3 & The 14 powers do not cluster homogeneously. & Supported & Silhouette $0.422$ at $k=3$; ARI $=1.0$ vs all three linkages (Ward, complete, average); MDS distance-rank $\rho=0.911$. \\
H4 & The ranking is robust to perturbation. & Partial & Equal-vs-PCA, equal-vs-literature, and normalisation clear the $0.85$ bar (CI lower bounds $0.877$/$0.877$/$0.861$); only the classifier (CI lower bound $0.845$) sits below it. \\
...
H6 & Top-5 and bottom-3 tier membership are stable. & Partial & $5/5$ top-5 members in tier in $\geq70\%$ of draws; no bottom-3 member reaches $80\%$, Israel only $49.4\%$, cycling with Saudi Arabia. \\
\bottomrule
\end{tabularx}
\end{table}
...
\subsection{Preparedness-checklist readiness}\label{subsec:readiness}
The counterfactuals and minimal-action sets above exercise four direct policy levers. The working paper's $\S$4, however, is a nine-action preparedness checklist---Detection (foresight, monitoring, verification), Escalation (severity schema, mandatory reporting, cross-sectoral pathways), and Mitigation \& Containment (emergency channels, defensive capabilities, contingency/kill-switch)---that tags each action with the \emph{enablers} it requires, arguing that ``the tractability of these actions depends upon a country's position within the 3 axes.'' We operationalise that general claim with an \emph{enabler-readiness matrix} (Figure~\ref{fig:readiness}): each checklist action's paper-assigned enablers are transcribed verbatim, each enabler is mapped to the M-PAPI indicators (or whole axis) that proxy it through an analyst-coded crosswalk shipped as an auditable ledger (\texttt{outputs/enabler\_crosswalk\_ledger.csv}), and a per-country readiness score is the mean of the country's literature-scheme $z$-scores on the \emph{proxied} enablers, reported alongside a coverage fraction.

The matrix makes the paper's heterogeneity claim concrete: top-tier countries (UK, Korea, France, EU, Japan) read broadly green because the enablers each action needs fall on axes where they already score highly, whereas capacity- or infrastructure-thin countries (India, Taiwan) are green only on the actions whose enablers sit on their stronger axis. Crucially, two of the paper's enablers---\emph{inter-disciplinary expertise} and \emph{intra-governmental coordination}---have no faithful country-level indicator; they are reported as \emph{un-proxied} (hatched in Figure~\ref{fig:readiness}, lowering the affected actions' coverage) rather than imputed, so the layer neither overstates coverage nor fabricates a score it cannot support.

\begin{figure}[t]
\centering
\includegraphics[width=0.97\textwidth]{figures/fig13_9_readiness_heatmap.png}
\caption{$\S$4 preparedness-checklist structural readiness per country: the mean literature-scheme $z$-score over the indicators proxying each checklist action's required enablers. Rows ordered by composite rank; thick rules separate Detection, Escalation, and Mitigation \& Containment; hatching marks actions with un-proxied enablers (coverage $<1$). A structural illustration, not a causal claim (\S\ref{subsec:causal}). Source: \texttt{outputs/preparedness\_readiness\_matrix.csv}.}
\label{fig:readiness}
\end{figure}

\subsection{Causal-interpretation boundary}\label{subsec:causal}
The counterfactuals (\S\ref{subsec:mas} and Figure~\ref{fig:cf}) are \emph{mechanical recomputations of the index} under a hypothetical indicator change---they answer ``if indicator $X$ took value $x'$, the composite would move by $\Delta$''---not estimates of the causal effect of a policy on real-world preparedness, and we do not present them as such. Reading a counterfactual $\Delta$ causally would require, in the language of Pearl's do-calculus \citep{pearl2009causal} and the potential-outcomes framework \citep{hernan2020causal}, a well-defined intervention, no unmeasured confounding, no interference between units, temporal precedence, and structural stability---none of which a single cross-sectional snapshot of $n = 14$ jurisdictions satisfies: AISI membership and strategy comprehensiveness co-vary with latent state capacity (the C5$\times$G1 $r = 0.965$ signal), one country's action shifts others' \emph{relative} ranks, and cause and outcome are observed simultaneously (Figure~\ref{fig:dag}; the full assumptions ledger is in \texttt{outputs/causal\_assumptions.csv}). The counterfactuals are therefore index-structural sensitivity probes and tractability illustrations, consistent with $\S$4's qualitative framing, while the paper's $\S$3 trigger-event/causal claims remain explicitly out of scope pending country-year incident data (Section~\ref{sec:limits}). The preparedness-checklist readiness matrix (\S\ref{subsec:readiness}) is governed by the same boundary: it reports where each country stands on the indicators proxying each action's required enablers, not whether the action will be executed or whether realised preparedness follows.
...
\subsection{Typology}
The silhouette-selected $k=3$ $k$-means partition separates a Tier-1/asymmetric-developed group, a mid group, and India as a singleton; the partition is identical to all three hierarchical linkages (Ward, complete, and average) at ARI $=1.0$. The MDS embedding (Figure~\ref{fig:mds}) preserves the pairwise-distance rank at $\rho=0.911$ and visually separates the archetypes.

\begin{figure}[t]
\centering
\includegraphics[width=0.86\textwidth]{figures/fig14_3_mds_embedding.png}
\caption{Multidimensional-scaling embedding of the 14 jurisdictions in the full fourteen-indicator space (distance-rank $\rho=0.911$). Colour: archetype; marker shape: AISI Network membership. Source: \texttt{outputs/mds\_coordinates.csv}.}
\label{fig:mds}
\end{figure}
...
\paragraph{It grounds the $\S$1 premise.} The paper claims the fourteen middle powers ``differ along three axes.'' H3 confirms this empirically: the countries do not cluster homogeneously. The silhouette-selected $k=3$ $k$-means partition---an eight-country developed group, a five-country mixed group, and India as a singleton---is identical to all three hierarchical linkages (Ward, complete, and average) at ARI $=1.0$ (Section~\ref{sec:results}). Reading each country instead by its own axis $z$-profile (a per-country labelling distinct from the $k$-means cluster id) yields four interpretable archetypes: Tier-1 (UK, Korea, France, Japan, Singapore), capacity-leveraged (EU, Canada, Germany), infrastructure-leveraged (Sweden, UAE), and Tier-3 vulnerable (Israel, Saudi Arabia, India, Taiwan)---the archetypes colour-coded in Figures~\ref{fig:typology} and~\ref{fig:mds}.
...
\paragraph{It quantifies $\S$4 tractability.} The paper repeats that ``tractability depends on a country's position within the three axes'' without specifying any action's leverage. The counterfactual scenarios (Figure~\ref{fig:cf}) convert this into a per-country, per-action quantity: the highest-leverage step differs by starting position, exactly as the paper's qualitative argument implies. The enabler-readiness matrix (Figure~\ref{fig:readiness}) generalises this from the four levers to all nine checklist actions, scoring each country's structural readiness on the indicators that proxy each action's required enablers---and reporting honestly which enablers (inter-disciplinary expertise; intra-governmental coordination) the fourteen indicators cannot proxy.
...
\textbf{Construct validity.} Several indicators are disclosed proxies whose link to the axis label is indirect: ND-GAIN readiness (I5) is a climate-adaptation measure used as an infrastructure-resilience proxy; V-Dem (G3) measures political-system democracy, not AI governance \emph{per se}; the IGSC count (G5) measures industrial biosecurity participation; and secure-server density (I3) increasingly reflects certificate-registration jurisdiction rather than substantive infrastructure. These are the most pressable choices; the defence is that the axes are the paper's, the proxies are transparent, and the ranking is robust to dropping any single indicator. The working paper's third axis is \emph{Compute and Infrastructure Posture}; the headline axis is built from ICT-infrastructure measures (I1--I3, I5), and a dedicated compute indicator is sourced and tested in Appendix~\ref{app:robust} (the TOP500 supercomputer-system count, attributed by installation country, so distinct from C2's organisation-attributed training compute---the overlap that ruled out an earlier Epoch-based candidate). Adding it leaves the ranking essentially unchanged (Spearman $\rho=0.996$; only an EU--Japan swap at ranks 4--5) but tips the silhouette-selected $k$ to 2, where average-linkage clustering degenerates and the pre-registered H3 criterion would fail on that artefact; it is therefore reported as a robustness check rather than folded into the headline. A hosting-side performance (Rmax) variant and coverage of unsubmitted private clusters remain future work (TOP500 is voluntary-submission).

\textbf{Cross-axis collinearity.} C5 (AISI presence) and G1 (AI-strategy comprehensiveness) are near-collinear ($r=0.965$) yet sit in different axes, so a shared institutional signal enters the composite twice. Reassigning C5 to governance leaves the top-five set unchanged (Spearman $\rho=0.982$, maximum single-country shift~1), so the double-count does not re-order the table. The exact second-order Shapley interaction index puts a magnitude on the overlap \emph{inside} the composite: $I(\text{C5},\text{G1})=+0.009$---small and mildly \emph{complementary}, not substitutive. The high correlation is thus a shared-information property (the two indicators co-vary across countries) rather than a large additive redundancy in the aggregated score; because C5 and G1 sit in different axes, the cross-axis geometric mean makes their joint marginal contribution slightly positive. The genuinely substitutive interactions are all within-axis pairs (e.g.\ C1$\times$C2 $=-0.021$), exactly as linear within-axis aggregation of correlated indicators predicts (Appendix~\ref{app:interp}). The double-count remains an acknowledged construct flaw---two indicators of one latent institutional-capacity signal---but its measured aggregate effect is bounded.

\textbf{Weighting and aggregation.} The vector-overlay weights are authorial translations of the paper's qualitative arguments, so the per-vector rankings are the most judgment-dependent output. The across-axis geometric mean introduces a shift constant $\varepsilon$: an $\varepsilon$-grid spanning $10^{-4}$ to $1$ shows the ranking is $\varepsilon$-invariant (Spearman $\rho=1.000$ for $\varepsilon\leq0.01$, $0.987$ at $\varepsilon=0.1$, $0.982$ at $\varepsilon=1$), but the bottom-tier composite \emph{values} are an $\varepsilon$-artefact (Taiwan's value spans $0.024$ to $1.56$ across the grid). The bottom tier should therefore be cited as ranks, not magnitudes. More generally, because the geometric shift references each axis's sample minimum, every composite \emph{value} is defined relative to the fourteen-country frame and the worst performer within it; the \emph{ranking} is the portable quantity, and composite magnitudes should not be read as cardinal, frame-independent preparedness levels.

\textbf{Inference and external validity.} With $n=14$, point-estimate rank correlations carry non-negligible sampling variance, which the Bonett--Wright CIs make explicit. The fourteen units are also not mutually independent: the European Union row is aggregated from member states that include three of the other thirteen units (France, Germany, and Sweden), so the $z$-score normalisation, the PCA loadings, the clustering, and the cross-axis correlations all rest on an overlapping-units assumption that a strict inferential reading would violate. We treat the fourteen as the paper's named census rather than a random sample (Section~\ref{sec:discussion}), and the leave-one-country-out jackknife bounds how much the (partly synthetic) EU row---or any single unit---drives the ranking and the typology; the non-independence nonetheless remains a caveat on every distributional statistic reported here. The sample is the fourteen named middle powers only; results do not generalise to non-middle-powers or small states, and composite values are not portable outside the fourteen-country reference frame. The ranking does, however, agree strongly with the independent Oxford Insights GAIRI 2024 index over the thirteen jurisdictions both cover (Spearman $\rho=0.852$; Section~\ref{subsec:extval}), which bounds the concern that the ordering is an artefact of the authors' own weighting.

% ============================================================
\section{Conclusion}
% ============================================================
M-PAPI converts the working paper's qualitative three-axes framework into a transparent, reproducible composite indicator over the paper's own fourteen named middle powers. It empirically positions each country, confirms that the countries are differentiable, identifies the most-exposed tier, quantifies the per-country leverage of preparedness actions, and scores structural readiness across the $\S$4 checklist---while honestly declining the trigger-event claims it cannot yet test. The headline ranking is robust to weighting and normalisation and borderline only to a single indicator-construction choice; the hypothesis verdicts (two fully, four partially supported) are reported with their specific caveats; and, within a pinned environment, every output regenerates byte-for-byte. The study is offered as auditable empirical scaffolding for the working paper, and as a template that can be re-run as the underlying data and the threat landscape evolve.

% ============================================================
\section*{Data and code availability}
% ============================================================
The notebook, its outputs and figures, the bundled offline data snapshot, and the source of this paper are openly available at \url{https://github.com/khailapvuong/middle-powers-mpapi} --- code under the MIT licence, bundled data under the respective upstream licences listed in Appendix~\ref{app:data}. All results regenerate from the bundled snapshot offline (\texttt{MPAPI\_OFFLINE=1}; Python~3.11+, \texttt{pip install -r requirements.txt}); a pinned \texttt{requirements.lock} reproduces the figures byte-for-byte, and a continuous-integration workflow re-executes the notebook on every commit and fails on any discrepancy with the documented hypothesis verdicts. Two source reports under restrictive redistribution terms (the ITU ICT Development Index report and the Stanford AI Index chapter) are not redistributed; the derived indicator values, the extraction scripts, and the source URLs are provided in their place.

\section*{Author contributions}
All authors developed the three-axes framework operationalised here in the parent working paper. The composite-indicator implementation, the sensitivity and robustness analysis, the reproducibility pipeline, and this companion paper were prepared by the author team; all authors reviewed and approved the final manuscript.
...
% ============================================================
Table~\ref{tab:vuln} gives the full per-vector vulnerability ranking. Figure~\ref{fig:rankdist} gives the Monte Carlo rank distributions; Figure~\ref{fig:withinaxis} the within-axis indicator correlations; Figure~\ref{fig:crossaxis} the full indicator correlation matrix; Figure~\ref{fig:dendro} the hierarchical clustering under three linkage methods; Figures~\ref{fig:axisheat}, \ref{fig:scatter}, and \ref{fig:govbiplot} the axis scoreboard, pairwise axis scatter matrix, and sparse-PCA governance sub-axes respectively; Figures~\ref{fig:loocountry} and~\ref{fig:rankagg} the leave-one-country-out jackknife and the weighting-free consensus ranking; and Figure~\ref{fig:compute} the TOP500 compute-indicator robustness check.

The $\varepsilon$-sensitivity, C5-reassignment, and analyst-coded judgment-collapse checks all leave the top-five set intact: the analyst-collapse variants (G1 and C5 individually and both at once) return min Spearman $\rho=0.996$ with at most a single mid-table shift, and the C5-reassignment returns $\rho=0.982$. Swapping the across-axis geometric mean for an arithmetic mean of the same min-shifted axis scores returns $\rho=0.978$ ($95\%$ CI $[0.911,0.995]$, maximum single-country shift two), with the top three invariant and the movement concentrated in the middle and lower tier where the geometric min-shift floor bites. Cross-axis correlations at $|r|\geq0.7$ number five of sixty-five pairs (C5$\times$G1 $0.965$; C1$\times$G1 $0.781$; C4$\times$I3 $-0.748$; G3$\times$I3 $0.722$; C3$\times$G6 $0.702$). Tier-membership stability over $10{,}000$ Monte Carlo draws: all five top-5 members in tier $\geq70\%$ of draws (UK $98.6\%$, Korea $97.9\%$, France $86.8\%$, Japan $83.3\%$, EU $76.8\%$); no bottom-3 member reaches $80\%$ (India $75.0\%$, Taiwan $60.9\%$, Israel $49.4\%$); exact-set-match $51\%/23\%$ and within-one-swap $93\%/66\%$ for the top-5/bottom-3.

The leave-one-country-out jackknife (Figure~\ref{fig:loocountry}) and the weighting-free consensus ranking (Figure~\ref{fig:rankagg}) corroborate the headline: dropping any single jurisdiction moves the retained thirteen by at most three rank positions (retained-order Spearman $\rho \geq 0.945$; typology ARI $=1.0$ for twelve of fourteen drops, falling to $0.61$ (India) and $0.47$ (Taiwan) for the other two), and a consensus that uses no cardinal weights tracks the literature ordering at Borda $\rho = 0.977$ and Kemeny--Young $\rho = 0.965$.

\paragraph{Compute-indicator robustness.} The working paper's third axis is named \emph{Compute and Infrastructure Posture}; we source a dedicated, hosting-attributed compute indicator---each country's count of TOP500 supercomputers \citep{top500_2025} (66th list, November 2025, aggregated by installation country, so distinct from C2's organisation-attributed training compute)---and test folding it into the infrastructure axis (Figure~\ref{fig:compute}). It is well-populated across all fourteen (minimum: Israel, three systems; no zeros). Adding it leaves the headline essentially unchanged: Spearman $\rho=0.996$ against the literature ranking ($95\%$ CI $[0.981,0.999]$), maximum single-country shift one, with only an EU$\leftrightarrow$Japan swap at ranks 4--5. We report it as a robustness check rather than folding it into the headline because doing so tips the silhouette-selected $k$ from 3 to 2, at which the $k$-means partition matches Ward and complete linkage exactly (ARI $=1.0$) but average linkage degenerates to a single-outlier (India) split (ARI $\approx0.05$), mechanically failing the pre-registered H3 min-ARI criterion despite strong clustering. The compute dimension is thus sourced, validated, and disclosed while the pre-registered headline stays stable.

\begin{figure}[t]
\centering
\includegraphics[width=0.7\textwidth]{figures/fig12_12_compute_sensitivity.png}
\caption{Compute-indicator robustness: the headline ranking versus the ranking with a TOP500 compute indicator (I4) added to the infrastructure axis. The only change is an EU$\leftrightarrow$Japan swap at ranks 4--5 (Spearman $\rho=0.996$ vs the headline). Source: \texttt{outputs/compute\_indicator\_sensitivity.csv}.}
\label{fig:compute}
\end{figure}
...
\midrule
United Kingdom        &  0.71 & 1  &  0.84 & 1  &  0.71 & 1 \\
South Korea           &  0.70 & 2  &  0.77 & 2  &  0.65 & 2 \\
Singapore             &  0.50 & 3  &  0.30 & 6  &  0.55 & 3 \\
European Union        &  0.48 & 4  &  0.58 & 4  &  0.23 & 6 \\
France                &  0.46 & 5  &  0.62 & 3  &  0.38 & 4 \\
Japan                 &  0.32 & 6  &  0.36 & 5  &  0.32 & 5 \\
Canada                &  0.01 & 7  &  0.08 & 7  & -0.13 & 9 \\
Germany               & -0.02 & 8  &  0.01 & 8  & -0.04 & 8 \\
Sweden                & -0.11 & 9  & -0.32 & 9  &  0.17 & 7 \\
United Arab Emirates  & -0.27 & 10 & -0.49 & 10 & -0.17 & 10 \\
Saudi Arabia          & -0.43 & 11 & -0.55 & 11 & -0.37 & 11 \\
Israel                & -0.63 & 12 & -0.72 & 13 & -0.64 & 13 \\
Taiwan                & -0.69 & 13 & -0.91 & 14 & -0.46 & 12 \\
India                 & -1.02 & 14 & -0.58 & 12 & -1.19 & 14 \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{Hierarchical clustering at $k=3$ under Ward, complete, and average linkage. All three linkage partitions (Ward, complete, average) are identical to the $k$-means partition (ARI $=1.0$). Source: \texttt{outputs/hierarchical\_ari.csv}.}
\label{fig:dendro}
\end{figure}
...
% ============================================================
All eight stochastic steps are seeded (\texttt{SEED $=$ 20260506}): the Monte Carlo weights, Horn's parallel-analysis null, the joint rank-uncertainty Monte Carlo, the per-vector vulnerability-overlay Monte Carlo, $k$-means, MDS, sparse PCA, and permutation importance. The leave-one-country-out re-clustering (\S\ref{subsec:mas} companion in \S\ref{sec:methods}) reuses the same seeded $k$-means; the Borda count and minimal-action-set enumeration are exact, and the Kemeny--Young median ranking is solved exactly by \texttt{scipy.optimize.milp} (HiGHS), deterministic within the pinned environment. With a pinned environment and the offline data snapshot (\texttt{MPAPI\_OFFLINE=1}), the forty output files (thirty-seven CSV, three JSON) and all twenty-five figures reproduce byte-for-byte across successive executions, verified by hashing two consecutive offline runs. The notebook additionally reconstructs one country's composite end-to-end (asserted within $10^{-3}$) and verifies Shapley additivity (within $10^{-6}$). The analysis is built on the open scientific Python stack \citep{harris2020numpy,mckinney2010pandas,virtanen2020scipy,pedregosa2011scikit,hunter2007matplotlib,waskom2021seaborn}.
