...
Table~\ref{tab:axes} reproduces the paper's axis definitions and lists the indicators assigned to each. The fifteen indicators comprise five \emph{capacity-depth} measures, six \emph{governance-orientation} measures, and four \emph{infrastructure-posture} measures.
...
Governance Orientation & AI-governance maturity, alliance posture, bilateral lab agreements & G1 national AI strategy; G2 ITU GCI; G3 V-Dem liberal democracy; G4 lab-MoU count; G5 IGSC member firms; G6 Australia Group \\
...
G4 & Bilateral lab-MoU count (coded) & Governance & none & 0.10 \\
...
The raw $14\times15$ matrix is $91.9\%$ observed ($193$ of $210$ cells). Figure~\ref{fig:missing} maps every cell to one of three states: observed, stage-1 axis-mean imputed, or stage-2 column-mean fallback. The largest gap is C4 (AI patents), missing for seven of the fourteen jurisdictions because they fall below the Stanford AI Index's published top-15 cut-off; C4 therefore exceeds the imputation threshold and its missing cells are column-mean filled. Taiwan's four infrastructure indicators are unobserved (a missing-not-at-random exclusion from ITU and World Bank ICT collection) and are likewise column-mean filled---a caveat carried through every downstream figure.

\begin{figure}[t]
\centering
\includegraphics[width=0.96\textwidth]{figures/fig5_4_missingness_heatmap.png}
\caption{Per-cell observability of the $14\times15$ raw indicator matrix. Blue: observed; amber: stage-1 axis-mean imputed; red: stage-2 column-mean fallback (missing-not-at-random). C4 patents (seven jurisdictions) and Taiwan's four infrastructure cells dominate the red region. Source: \texttt{outputs/missingness\_audit.csv}.}
\label{fig:missing}
\end{figure}
...
\midrule
1  & United Kingdom        & 2.227 & 2.571 & 2.314 \\
2  & South Korea           & 1.951 & 1.998 & 2.185 \\
3  & France                & 1.792 & 2.041 & 1.980 \\
4  & European Union        & 1.785 & 2.126 & 1.824 \\
5  & Japan                 & 1.725 & 1.897 & 1.823 \\
6  & Singapore             & 1.418 & 1.549 & 1.809 \\
7  & Canada                & 1.446 & 1.837 & 1.472 \\
8  & Germany               & 1.285 & 1.335 & 1.434 \\
9  & Sweden                & 0.958 & 0.978 & 1.157 \\
10 & United Arab Emirates  & 0.673 & 0.307 & 1.020 \\
11 & Saudi Arabia          & 0.628 & 0.116 & 0.922 \\
12 & Israel                & 0.100 & 0.661 & 0.144 \\
13 & India                 & 0.073 & 0.067 & 0.106 \\
14 & Taiwan                & 0.065 & 0.060 & 0.057 \\
\bottomrule
\end{tabular}
\end{table}
...
Under the Bonett--Wright standard error, the three weighting/normalisation perturbations clear the $0.70$ robustness floor with wide margins (Table~\ref{tab:robust}, Figure~\ref{fig:forest}). The bibliographic-classifier check has a point estimate of $\rho=0.960$ but a $95\%$ CI lower bound of $0.845$, which clears the $0.70$ floor yet sits marginally below the stricter $0.85$ bar imposed on that check (a core measurement choice rather than a downstream weight perturbation). The Monte Carlo rank distributions (Appendix Figure~\ref{fig:rankdist}) show the United Kingdom tightest at rank~1 and the $\{$France, EU, Japan$\}$ block sharing a median rank of~4.
...
Weighting: equal vs literature     & 0.996 & 0.70 & $[0.982,\,0.999]$ & clears \\
Normalisation: $z$ vs min--max     & 0.996 & 0.70 & $[0.982,\,0.999]$ & clears \\
...
\caption{Robustness forest plot: Spearman $\rho$ point estimates with Bonett--Wright Fisher-$z$ $95\%$ CIs against each perturbation's pass threshold (dashed). The three weighting/normalisation checks clear $0.70$ with wide margins; the classifier-sensitivity check is borderline against its stricter $0.85$ bar. Source: \texttt{outputs/robustness\_summary\_with\_ci.csv}.}
\label{fig:forest}
\end{figure}

\paragraph{Rank uncertainty.} The joint Monte Carlo---perturbing weights \emph{and} imputed cells while holding observed values fixed---attaches an uncertainty band to every position. Under the most conservative setting ($\sigma_2=1.0$), the United Kingdom holds rank~1 in $99\%$ of draws and South Korea sits in the top five in $92\%$; the bottom tier is comparably stable (India and Taiwan in the bottom three in $82\%$ and $78\%$ of draws). The added imputation noise widens the credible intervals only marginally over the weight-only Monte Carlo (Figure~\ref{fig:rankunc}), confirming that the headline ordering is not an artefact of the fixed imputed values---the cells the pipeline had to fill do not drive the ranking. The middle of the table is where the bands overlap most, consistent with the single-slot instability behind H6's partial verdict.
...
Table~\ref{tab:hyp} records the six hypotheses, their numeric pass criteria, and their mechanically resolved verdicts. Two are fully supported (H3, H5) and four are partially supported with named caveats (H1, H2, H4, H6). The partial verdicts surface specific methodological caveats---a near-collinear pair across two axes (C5$\times$G1, Pearson $r=0.973$), governance multidimensionality, the borderline classifier check, and single-slot tier cycling---rather than a wholesale framework failure. The C5$\times$G1 collinearity is the largest of only five cross-axis indicator pairs at $|r|\geq0.7$ (of 74 pairs).
...
\midrule
H1 & The three axes are empirically separable. & Partial & $5$ of $74$ cross-axis pairs at $|r|\geq0.7$; C5$\times$G1 ($r=0.973$) is the largest contributor. \\
H2 & Each axis carries a one-dimensional signal. & Partial & Infrastructure $p=0.014$, governance $p=0.025$ (both significant); capacity untestable at $n=7$; governance PC1 explains $42.5\%$. \\
H3 & The 14 powers do not cluster homogeneously. & Supported & Silhouette $0.426$ at $k=3$; ARI $=1.0$ vs Ward and average linkage; MDS distance-rank $\rho=0.907$. \\
H4 & The ranking is robust to perturbation. & Partial & Weighting/normalisation clear with wide margins; classifier CI lower bound $0.845$ is below its $0.85$ bar. \\
H5 & The top-5 is not driven by any one coded indicator. & Supported & Top-5 set unchanged dropping any of G1/G4/C5; max single-country shift $2$. \\
H6 & Top-5 and bottom-3 tier membership are stable. & Partial & $5/5$ top-5 members in tier in $\geq70\%$ of draws; Israel only $62.2\%$ in the bottom-3, cycling with Saudi Arabia. \\
\bottomrule
\end{tabularx}
\end{table}
...
To operationalise the paper's $\S$4 claim that tractability depends on axis position, we recompute the composite under five direct policy actions per country (e.g.\ joining the AISI Network, publishing a safety-provisioned AI strategy, joining the Australia Group). Figure~\ref{fig:cf} reports the composite change; the highest-leverage action differs by country and by starting position. For example, with Singapore selected, joining the Australia Group raises its composite from $1.809$ to $1.911$ (rank 6 to 4). For countries at an axis minimum, the geometric-shift artefact can understate the composite change, so an axis-level reading is reported alongside.
...
\subsection{Typology}
The silhouette-selected $k=3$ $k$-means partition separates a Tier-1/asymmetric-developed group, a mid group, and India as a singleton; the partition is identical to Ward and average-linkage hierarchical clustering (ARI $=1.0$; complete linkage $0.733$). The MDS embedding (Figure~\ref{fig:mds}) preserves the pairwise-distance rank at $\rho=0.907$ and visually separates the archetypes.

\begin{figure}[t]
\centering
\includegraphics[width=0.86\textwidth]{figures/fig14_3_mds_embedding.png}
\caption{Multidimensional-scaling embedding of the 14 jurisdictions in the full fifteen-indicator space (distance-rank $\rho=0.907$). Colour: archetype; marker shape: AISI Network membership. Source: \texttt{outputs/mds\_coordinates.csv}.}
\label{fig:mds}
\end{figure}
...
% ============================================================
Table~\ref{tab:vuln} gives the full per-vector vulnerability ranking. Figure~\ref{fig:rankdist} gives the Monte Carlo rank distributions; Figure~\ref{fig:withinaxis} the within-axis indicator correlations; Figure~\ref{fig:crossaxis} the full indicator correlation matrix; Figure~\ref{fig:dendro} the hierarchical clustering under three linkage methods; Figures~\ref{fig:axisheat}, \ref{fig:scatter}, and \ref{fig:govbiplot} the axis scoreboard, pairwise axis scatter matrix, and sparse-PCA governance sub-axes respectively.

The $\varepsilon$-sensitivity, C5-reassignment, and analyst-coded judgment-collapse checks all leave the top-five set intact: the analyst-collapse variants (G1, C5, G4 individually and all three at once) each return Spearman $\rho=1.000$ with zero rank shift, and the C5-reassignment returns $\rho=0.996$. Cross-axis correlations at $|r|\geq0.7$ number five of seventy-four pairs (C5$\times$G1 $0.973$; C1$\times$G1 $0.779$; C4$\times$I3 $-0.771$; G3$\times$I3 $0.722$; C3$\times$G6 $0.702$). Tier-membership stability over $10{,}000$ Monte Carlo draws: top-5 members in tier $\geq80\%$ of draws (UK $99.0\%$, Korea $93.2\%$, France $83.8\%$, Japan $82.5\%$, EU $80.5\%$); bottom-3 members India $82.0\%$, Taiwan $81.8\%$, Israel $62.2\%$; exact-set-match $46\%/43\%$ and within-one-swap $94\%/83\%$ for the top-5/bottom-3.
...
\midrule
United Kingdom        &  0.73 & 1  &  0.89 & 1  &  0.76 & 1 \\
South Korea           &  0.70 & 2  &  0.76 & 2  &  0.62 & 2 \\
Singapore             &  0.48 & 3  &  0.27 & 6  &  0.54 & 3 \\
European Union        &  0.48 & 4  &  0.58 & 4  &  0.24 & 6 \\
France                &  0.43 & 5  &  0.59 & 3  &  0.36 & 4 \\
Japan                 &  0.32 & 6  &  0.36 & 5  &  0.32 & 5 \\
Canada                &  0.08 & 7  &  0.18 & 7  & -0.08 & 9 \\
Germany               & -0.00 & 8  &  0.02 & 8  & -0.05 & 8 \\
Sweden                & -0.12 & 9  & -0.34 & 9  &  0.14 & 7 \\
United Arab Emirates  & -0.24 & 10 & -0.44 & 10 & -0.14 & 10 \\
Saudi Arabia          & -0.42 & 11 & -0.53 & 11 & -0.35 & 11 \\
Israel                & -0.63 & 12 & -0.72 & 13 & -0.64 & 13 \\
Taiwan                & -0.77 & 13 & -1.01 & 14 & -0.51 & 12 \\
India                 & -1.03 & 14 & -0.60 & 12 & -1.21 & 14 \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{Full $15\times15$ indicator correlation matrix with axis-block separators; $|r|\geq0.5$ annotated. Diagonal blocks are within-axis; off-blocks are cross-axis. Source: \texttt{outputs/cross\_axis\_correlations.csv}.}
\label{fig:crossaxis}
\end{figure}
...
\caption{Hierarchical clustering at $k=3$ under Ward, complete, and average linkage. Ward and average partitions are identical to the $k$-means partition (ARI $=1.0$). Source: \texttt{outputs/hierarchical\_ari.csv}.}
\label{fig:dendro}
\end{figure}
...
% ============================================================
Figure~\ref{fig:shap} decomposes each country's composite into exact Shapley contributions per indicator (all $2^{15}$ coalitions; additivity verified to $10^{-15}$). Figure~\ref{fig:perm} reports permutation feature importance for the literature ranking. Training compute (C2) and notable-model count (C1) are the most rank-disruptive indicators; ND-GAIN readiness (I5) and patents (C4) the least. Figure~\ref{fig:shapint} reports the exact second-order Shapley interaction indices \citep{grabisch1999interaction} for every indicator pair, averaged over the fourteen countries: the within-axis pairs are the substitutive (negative) ones, while cross-axis pairs---including the much-discussed C5$\times$G1 ($+0.008$)---are weakly complementary, locating indicator redundancy firmly within rather than across axes.
